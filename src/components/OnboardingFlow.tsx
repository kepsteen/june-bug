...
import { useMutation as useConvexMutation } from '@convex-dev/react-query'
...
interface OnboardingData {
  ...
  customTime: string
  notificationPreferences: Array<'Push notifications' | 'Email reminders' | 'None'>
}
...
export default function OnboardingFlow({ userId }: OnboardingFlowProps) {
  ...
  const completeOnboardingMutation = useConvexMutation(api.onboarding.completeOnboarding)
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        if (!formData.fullName.trim() || !formData.currentRole.trim() || !formData.experienceLevel) {
      ...
      case 2:
        ...
        const hasGoals = formData.developmentGoals.length > 0 || formData.customGoal.trim()
        if (!hasGoals) {
        ...
        return true
      case 3:
        if (formData.techStack.length === 0 && !formData.customTechStack.trim()) {
      ...
      case 4:
        ...
        if (formData.journalingFrequency === 'Custom schedule' && formData.customScheduleDays.length === 0) {
  ...
  const handleComplete = async () => {
    try {
      ...
      await completeOnboardingMutation({
        ...
        customScheduleDays: formData.journalingFrequency === 'Custom schedule'
  ...
  if (showSummary) {
    return (
      <div className="container max-w-2xl mx-auto px-4 py-8">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">Let's Review Your Profile</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {formData.age && `, ${formData.age} years old`} • {formData.experienceLevel}{' '}
                {formData.currentRole} • {formData.workEnvironment}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[...formData.developmentGoals, ...(formData.customGoal.trim() ? [formData.customGoal] : [])].map(
                  (goal) => (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[...formData.techStack, ...(formData.customTechStack.trim() ? [formData.customTechStack] : [])].map(
                  (tech) => (
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                at {formData.customTime || formData.journalingTime} to reflect on your journey as a{' '}
                {formData.experienceLevel} {formData.currentRole}.
              </p>
              {formData.notificationPreferences.includes('None') ? (
                <p className="text-sm text-muted-foreground mt-2">
                  You've opted out of reminders - we trust you'll remember on your own!
                </p>
              ) : (
                <p className="text-sm text-muted-foreground mt-2">
                  Notifications via: {formData.notificationPreferences.join(', ')}
                </p>
              )}
            </div>
    ...
    <div className="container max-w-2xl mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-sm text-muted-foreground">About {estimatedMinutes} min remaining</span>
        </div>
        ...
        {currentStep === 1 && (
          <>
            <CardHeader>
              ...
              <CardDescription>
                Help us personalize your journaling experience and provide relevant guidance tailored to your journey
              </CardDescription>
            </CardHeader>
        ...
        {currentStep === 2 && (
          <>
            <CardHeader>
              ...
              <CardDescription>
                Understanding your learning style helps us provide better guidance and feedback
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-3">
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) => updateFormData({ mentorshipStyle: value as any })}
                >
                  ...
                  <div className="flex items-start space-x-2 p-3 rounded-lg border hover:bg-accent cursor-pointer">
                    <RadioGroupItem value="Challenge-driven" id="challenge" className="mt-1" />
                    <Label htmlFor="challenge" className="cursor-pointer flex-1">
                  ...
                  <div className="flex items-start space-x-2 p-3 rounded-lg border hover:bg-accent cursor-pointer">
                    <RadioGroupItem value="Reflective" id="reflective" className="mt-1" />
                    <Label htmlFor="reflective" className="cursor-pointer flex-1">
                  ...
                  {DEVELOPMENT_GOAL_OPTIONS.map((goal) => (
                    <div key={goal} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleArrayItem('developmentGoals', goal)}
                      />
        ...
        {currentStep === 3 && (
          <>
            <CardHeader>
              ...
              <CardDescription>
                This helps us provide relevant examples and suggestions in your reflections
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-3">
                  ...
                  {TECH_STACK_OPTIONS.map((tech) => (
                    <div key={tech} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleArrayItem('techStack', tech)}
                      />
                ...
                <div className="space-y-2 mt-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateFormData({ customTechStack: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => updateFormData({ workEnvironment: value as any })}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Team lead/manager">Team lead/manager</SelectItem>
                    <SelectItem value="Freelance/consultant">Freelance/consultant</SelectItem>
                    <SelectItem value="Student/bootcamp">Student/bootcamp</SelectItem>
                    <SelectItem value="Career transition/job seeking">Career transition/job seeking</SelectItem>
                    <SelectItem value="Side projects only">Side projects only</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </CardContent>
        ...
        {currentStep === 4 && (
          <>
            <CardHeader>
              ...
              <CardDescription>Set up your reflection routine to build consistent habits</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-3">
                <Label>
                  Journaling Frequency <span className="text-destructive">*</span>
                </Label>
                <RadioGroup
                  value={formData.journalingFrequency}
                  onValueChange={(value) => updateFormData({ journalingFrequency: value as any })}
                >
                  <div className="flex items-center space-x-2 p-3 rounded-lg border hover:bg-accent cursor-pointer">
                    <RadioGroupItem value="Daily" id="daily" />
                    <Label htmlFor="daily" className="cursor-pointer flex-1 flex items-center gap-2">
                      <span>Daily</span>
                  ...
                  <div className="flex items-center space-x-2 p-3 rounded-lg border hover:bg-accent cursor-pointer">
                    <RadioGroupItem value="Every other day" id="everyOtherDay" />
                    <Label htmlFor="everyOtherDay" className="cursor-pointer flex-1">
                      Every other day
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 p-3 rounded-lg border hover:bg-accent cursor-pointer">
                ...
                {formData.journalingFrequency === 'Custom schedule' && (
                  <div className="mt-4 p-4 rounded-lg border bg-muted/50">
                    <Label className="mb-3 block">Select days of the week</Label>
                    <div className="grid grid-cols-2 gap-2">
                      {DAYS_OF_WEEK.map((day) => (
                        <div key={day} className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={() => toggleArrayItem('customScheduleDays', day)}
                          />
              ...
              <div className="space-y-2">
                <Label htmlFor="journalingTime">
                  Preferred Journaling Time <span className="text-destructive">*</span>
                </Label>
                <Select
                  value={formData.journalingTime}
                  onValueChange={(value) => updateFormData({ journalingTime: value })}
                >
              ...
              <div className="space-y-3">
                <Label>
                  Notification Preferences <span className="text-destructive">*</span>
                </Label>
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      id="push"
                      checked={formData.notificationPreferences.includes('Push notifications')}
                      onCheckedChange={() => toggleArrayItem('notificationPreferences', 'Push notifications')}
                    />
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      id="email"
                      checked={formData.notificationPreferences.includes('Email reminders')}
                      onCheckedChange={() => toggleArrayItem('notificationPreferences', 'Email reminders')}
                    />
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      id="none"
                      checked={formData.notificationPreferences.includes('None')}
                      onCheckedChange={() => toggleArrayItem('notificationPreferences', 'None')}
                    />
